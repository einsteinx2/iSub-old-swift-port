...
- (void)startGlowingWithColor:(UIColor *)color intensity:(CGFloat)intensity animated:(BOOL)animated {
    [self startGlowingWithColor:color fromIntensity:0.1 toIntensity:intensity radius:10. overdub:0 animated:animated repeat:YES];
}
...
- (void) startGlowingWithColor:(UIColor*)color fromIntensity:(CGFloat)fromIntensity toIntensity:(CGFloat)toIntensity radius:(CGFloat)radius overdub:(NSUInteger)overdub animated:(BOOL)animated repeat:(BOOL)repeat {
    ...
    // Darken the glow by adding more layers
    for (int i = 1; i < overdub; i++)
    {
        UIView* extraGlow = [[UIImageView alloc] initWithImage:image];
        extraGlow.layer.shadowColor = color.CGColor;
        extraGlow.layer.shadowOffset = CGSizeZero;
        extraGlow.layer.shadowRadius = radius;
        extraGlow.layer.shadowOpacity = 1.0;
        [glowView addSubview:extraGlow];
    }
...
- (void) glowOnceAtLocation:(CGPoint)point inView:(UIView*)view {
    [self startGlowingWithColor:[UIColor whiteColor] fromIntensity:0 toIntensity:0.6 radius:10 overdub:0 animated:YES repeat:NO];
...
- (void) startGlowing
{
    [self startGlowingAnimated:NO];
}

// Create a pulsing, glowing view based on this one.
- (void) startGlowingAnimated:(BOOL)animated {
    [self startGlowingWithColor:[UIColor whiteColor] intensity:0.6 animated:(BOOL)animated];
}
