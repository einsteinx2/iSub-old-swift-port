...
static NSTimeInterval defaultTimeOutSeconds = 240;
...
+ (NSString *)errorCodeToEnglish:(int)errorCode;
{
	switch (errorCode) {
		case 1:
		...
		case 6:
			return @"Internal Error While Building Request";
		case 7:
			return @"Internal Error While Applying Credentials";
		case 8:
			return @"File Management Error";
		case 9:
			return @"Too Much Redirection";
		case 10:
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setValidatesSecureCertificate:NO]; // Don't validate SSL certs (this allows self signed certs)
...
//+ (BOOL)isNetworkReachableViaWWAN:(NSNotification *)note
+ (BOOL)isNetworkReachableViaWWAN
{
	//Reachability* curReach = [note object];
	//return ([curReach currentReachabilityStatus] == ReachableViaWWAN);
	iSubAppDelegate *appDelegate = (iSubAppDelegate *)[[UIApplication sharedApplication] delegate];
	return ([appDelegate.wifiReach currentReachabilityStatus] == ReachableViaWWAN);
}

+ (void)reachabilityChanged:(NSNotification *)note
{
	[bandwidthThrottlingLock lock];
	//isBandwidthThrottled = [ASIHTTPRequest isNetworkReachableViaWWAN:note];
	isBandwidthThrottled = [ASIHTTPRequest isNetworkReachableViaWWAN];
	NSLog(@"isBandwidthThrottled = %i", isBandwidthThrottled);
	[bandwidthThrottlingLock unlock];
}
#endif
